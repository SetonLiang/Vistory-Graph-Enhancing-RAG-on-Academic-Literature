...
class Entities(BaseModel):
    names: List[str] = Field(
        ...,
        description="All the Department, Author, keyword, and Papers entities that appear in the text",
    )
    ...
    for entity in entity_names:
        ...
        if keyword_response:
            for record in keyword_response:
                ...
                #     "papers": [p.get('name') for p in record['papers']],
        ...
        if department_response:
            ...
            for record in department_response:
                paper_count = len(record['papers']) #统计每个作者的paper数量
...
def return_response(question):
    ...
    entity_prompt = ChatPromptTemplate.from_messages([
        (
            "system", 
            " You are extracting Department, Author, keyword, and Papers entities from the text.",           
        ),
        (
            "placeholder",
            "{examples}"
...
if __name__ == "__main__":
    ...
    # 加载作者和论文数据
    import_dir = 'D://neo4j-community-5.20.0-windows/neo4j-community-5.20.0/import'
    ...
    # 运行QA链
    ans = chain.invoke({"question": "What's the department wei Zeng belongs to".lower()})
